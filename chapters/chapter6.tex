...
\textbf{BP2:} $\delta_j^{(l)} = \partial\mathcal{L}/\partial z_j^{(l)} =
\sum_k (\partial\mathcal{L}/\partial z_k^{(l+1)})
(\partial z_k^{(l+1)}/\partial a_j^{(l)})
(\partial a_j^{(l)}/\partial z_j^{(l)}) =
\sum_k \delta_k^{(l+1)} W_{kj}^{(l+1)} \sigma'^{(l)}(z_j^{(l)}) =
({\bm{W}^{(l+1)}}^\top\bm{\delta}^{(l+1)})_j \cdot \sigma'^{(l)}(z_j^{(l)})$.
